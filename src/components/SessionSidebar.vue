                ...
                >
                    <div class="file-toolbar">
                        <select v-model="fileSessionId" :disabled="availableSshSessions.length === 0">
                            ...
                            <option v-for="session in availableSshSessions" :key="session.sessionId" :value="session.sessionId">
                    ...
                    <div class="file-actions">
                        <IconButton title="上级目录" :disabled="!fileSessionId || remotePath === '/'" @click="goRemoteParent">
                        ...
                        <IconButton title="上传到当前目录" :disabled="!fileSessionId || sftpUploading" @click="uploadToRemote">
                    ...
                    />
                    <div v-if="sftpProgress.visible" class="file-progress" :class="{ error: sftpProgress.phase === 'error' }">
                        <div class="file-progress__line">
                            <span>{{ sftpProgressText }}</span>
                            <strong>{{ sftpProgressPercent }}</strong>
                        </div>
                        <div class="file-progress__bar">
                            <i :style="{ width: `${sftpProgressBarWidth}%` }"></i>
                        </div>
                    </div>
...
const activeMeta = computed(() => {
    if (activePanel.value === 'sessions') return `${store.sessionFolders.length} 文件夹 · ${store.sessions.length} 会话`;
...
const contextSession = computed(() => (contextNode.value?.type === RESOURCE_NODE_TYPES.SESSION ? contextNode.value : null));
const contextFolder = computed(() => (contextNode.value?.type === RESOURCE_NODE_TYPES.SESSION_FOLDER ? contextNode.value : null));
...
async function promptCreateFolder() {
    ...
    const parentId =
        parentNode?.type === RESOURCE_NODE_TYPES.SESSION_FOLDER
            ? parentNode.id
            : getCreateFolderId();
    closeFolderMenu();
