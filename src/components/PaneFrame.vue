    ...
    <div class="pane" :class="{ active: node.id === store.activePaneId }" @mousedown="focus">
        ...
        >
            ...
            <div v-if="sessions.length === 0" class="empty">
                ...
                <button class="ph-btn close-empty" v-if="!isOnlyPane" title="关闭标签组" @click.stop="close">关闭标签组</button>
            </div>

            <div v-if="edgeDrop" class="edge-marker" :class="edgeDrop"></div>
        </div>
    </div>
</template>
...
function tabTitle(session) {
    ...
    if (session.protocol === 'ssh') return `${session.name} - ssh://${session.username ? `${session.username}@` : ''}${session.host}:${session.port}`;
