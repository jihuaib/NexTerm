...
const nodeTitle = computed(() => {
    ...
    if (isSession.value && props.node.protocol === 'ssh') return `${props.node.name} - ssh://${props.node.username ? `${props.node.username}@` : ''}${props.node.host}:${props.node.port}`;
    if (isSession.value) return `${props.node.name} - ${props.node.protocol}://${props.node.host}:${props.node.port}`;
