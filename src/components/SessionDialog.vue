    ...
    >
        ...
        <div class="session-form-body">
            <div class="form-grid common-grid">
                <label>
                    <span>名称</span>
                    <input v-model="form.name" :placeholder="isLocal ? '可选，默认显示 Local Shell' : '可选，默认使用主机地址'" />
                </label>
            ...
            <div v-else class="form-grid type-grid">
                <label class="wide">
                    <span>Shell</span>
                    <input v-model="form.shell" placeholder="默认使用系统 Shell，例如 /bin/zsh、powershell.exe、cmd.exe" />
                </label>
...
const form = reactive({
    ...
    port: props.session?.port || defaultPortForProtocol(props.session?.protocol || store.settings.defaultProtocol, store.settings.defaultPort),
